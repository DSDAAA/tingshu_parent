...
class CateGory extends Service {
  getCategory1() {
    return this.get({
      url: '/api/album/category/getCategory1'
    })
  }
  /**
  ...
  findAllCategory() {
    return this.get<CategoryListInterface[]>({
      url: '/api/album/category/getAllCategoryList'
    })
  }
  /**
  ...
  getAttrList(category1Id: string | number) {
    return this.get<AttributeListInterface[]>({
      url: `/api/album/category/getPropertyByCategory1Id/${category1Id}`
    })
  }
  /**
  ...
  getCategory1IdData(id:string | number) {
    return this.get<ChannelInterface[]>({
      url: `/api/search/albumInfo/getChannelData/${id}`
    })
  }
  /**
  ...
  getCategory1IdTopList(category1Id:string | number) {
    return this.get<CategoryTreePropsInterface[]>({
      url: `/api/album/category/getCategory3ListByCategory1Id/${category1Id}`
    })
  }
  // 根据一级分类id获取全部分类信息
  getCategory1IdAllInfo(category1Id:string | number) {
    return this.get<CategoryTreeInterface>({
      url: `/api/album/category/getCategoryByCategory1Id/${category1Id}`
    })
  }
