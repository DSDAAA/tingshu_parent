		...
		methods: {
			...
			async onFieldChange(value, formtrigger = true) {
				...
				if (validateTrigger === 'bind' || formtrigger) {
					// 校验当前表单项
					retVal = await this.validator.validateUpdate({
							[name]: value
						},
						formData
					);
					...
					if (retVal && retVal.errorMessage) {
						if (errShowType === 'undertext') {
							// 获取错误信息
							this.errMsg = !retVal ? '' : retVal.errorMessage;
						}
						if (errShowType === 'toast') {
							uni.showToast({
								title: retVal.errorMessage || '校验错误',
								icon: 'none'
							});
						}
						if (errShowType === 'modal') {
							uni.showModal({
								title: '提示',
								content: retVal.errorMessage || '校验错误'
							});
						}
					} else {
						this.errMsg = ''
					...
					// 通知 form 组件更新事件
					validateCheck(retVal ? retVal : null)
				} else {
					this.errMsg = ''
				}
